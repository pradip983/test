    ...
    <>
      <div className="w-full h-[85vh]">
        <div className="w-[80%] h-[50vh] m-auto">
          <h1 className="text-black text-2xl font-bold font-sans p-1">Trending Destinations</h1>
          <h2 className="text-black text-xl font-sans p-1 ">
  ...
  <div className="grid grid-cols-2 gap-4">
    {firstTwoDestinations.map((ftd) => (
      <div
        ...
        className="h-[33vh] relative bg-cover bg-center rounded-lg shadow-md overflow-hidden transform transition-transform duration-300 hover:scale-90"
        style={{ backgroundImage: `url(${ftd.image})` }}
        onClick={() => navigateToDetails(ftd._id)}
      >
        <h1 className="absolute top-4 left-4 text-white text-xl font-bold shadow-md flex gap-3">
          <div>{ftd.title}</div>
          <div>
            <img src="/world.png" alt="" className="w-7" />
          </div>
        </h1>
      </div>
    ))}
  ...
  <div className="grid grid-cols-3 gap-4 mt-4">
    {lastThreeDestinations.map((ltd) => (
      <div
        ...
        className="h-[27vh] relative bg-cover bg-center rounded-lg shadow-md overflow-hidden transform transition-transform duration-300 hover:scale-90"
